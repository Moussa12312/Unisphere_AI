...
import { Eye, EyeOff, Mail, Lock, Check, Quote } from 'lucide-react';
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // ✅ NOUVEAU : Générer les notifications une seule fois au login
      try {
        await api.post('/api/v1/notifications/generate');
        console.log('✅ Notifications générées avec succès au login');
      } catch (notifError) {
        // On met juste un warning, on ne bloque PAS la connexion si ça échoue
        console.warn('⚠️ Erreur génération notifications (non bloquant):', notifError);
      }
  
      toast.success('Connexion réussie !');
  
      // ✅ CORRIGÉ : Ajout de accountant et guard
      const routes: Record<string, string> = {
        ...
        accountant: '/accountant/dashboard',  // ✅ AJOUTÉ
        guard: '/guard/dashboard',            // ✅ AJOUTÉ
      };
      ...
      <div className="w-90 lg:w-1/3 flex items-center justify-center p-5 relative z-10 mt-5 mr-8">
        <div className="w-full max-w-md bg-white rounded-3xl shadow-2xl p-8 lg:p-7">
          ...
          <div className="grid grid-cols-2 gap-3">
            <button className="flex items-center justify-center gap-2 border border-none rounded-xl hover:bg-slate-50 transition-colors">
