...
import { User, Mail, Phone, Shield, Save, Briefcase, DollarSign, TrendingUp, Users, Receipt } from 'lucide-react';
...
export default function AccountantProfilePage() {
  ...
  const fetchProfile = async () => {
    try {
      ...
      setUserData({
        ...
        role: response.data.role || 'accountant'
      });
    } catch (error) {
  ...
  const fetchStats = async () => {
    try {
      const [paymentsRes, studentsRes] = await Promise.all([
        api.get('/api/v1/payments/').catch(() => ({ data: [] })),
  ...
  return (
    <div className="space-y-6 pb-10">
      ...
      {/* Carte de profil */}
      <div className="bg-gradient-to-r from-emerald-600 to-teal-600 rounded-2xl p-6 text-white shadow-lg shadow-emerald-500/30">
        <div className="flex items-center gap-5">
          ...
          <div className="hidden md:flex gap-3">
            <div className="bg-white/20 backdrop-blur-sm rounded-xl p-3 text-center min-w-[120px]">
              ...
              <p className="text-lg font-bold">{formatFCFA(stats.total_payments)}</p>
              <p className="text-xs opacity-80">Total encaissé</p>
            </div>
        ...
        <div className="lg:col-span-2 bg-white rounded-xl border border-slate-200 shadow-sm p-6">
          ...
          <div className="space-y-4">
            ...
            <button
              ...
              disabled={saving}
              className="flex items-center gap-2 px-6 py-2.5 bg-emerald-600 text-white rounded-lg font-medium hover:bg-emerald-700 disabled:opacity-50 transition-colors shadow-md shadow-emerald-500/20"
            >
              {saving ? (
      ...
      <div className="md:hidden grid grid-cols-2 gap-3">
        <div className="bg-white rounded-xl border border-slate-200 p-4 text-center">
          ...
          <p className="text-lg font-bold text-slate-900">{formatFCFA(stats.total_payments)}</p>
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-4 text-center">
          ...
          <p className="text-2xl font-bold text-slate-900">{stats.pending_payments}</p>
