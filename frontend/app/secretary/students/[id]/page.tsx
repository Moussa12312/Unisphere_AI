  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] gap-4">
        ...
        <button onClick={loadStudent} className="bg-[#FF6B00] text-white px-4 py-2 rounded-lg hover:bg-[#e55f00] mt-4">Réessayer</button>
      </div>
    );
  }
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <div className="flex items-center gap-3">
            ...
            <span className={`text-xs font-medium px-2.5 py-1 rounded-full ${student.status === 'active' ? 'bg-green-100 text-green-700' : student.status === 'pending' ? 'bg-orange-100 text-orange-700' : 'bg-slate-100 text-slate-700'}`}>
        ...
        <div className="flex items-center gap-2">
          <Link href={`/secretary/students/${params.id}/edit`} className="bg-[#0a1628] hover:bg-slate-800 text-white px-4 py-2 rounded-lg text-sm font-medium flex items-center gap-2 transition-colors">
            ...
            {showMenu && (
              <>
                ...
                <div className="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg border border-slate-200 py-1 z-20">
                  <button onClick={() => { navigator.clipboard.writeText(student.email); toast.success('Email copié'); setShowMenu(false); }} className="w-full px-4 py-2 text-sm text-slate-700 hover:bg-slate-100 text-left flex items-center gap-2"><Mail size={14} /> Copier l'email</button>
                  <button onClick={() => { navigator.clipboard.writeText(student.matricule); toast.success('Matricule copié'); setShowMenu(false); }} className="w-full px-4 py-2 text-sm text-slate-700 hover:bg-slate-100 text-left flex items-center gap-2"><FileText size={14} /> Copier le matricule</button>
                </div>
              </>
            )}
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="space-y-5">
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
            <img src={photoUrl || avatarFallback} alt={`${student.first_name} ${student.last_name}`} className="w-32 h-32 rounded-full mx-auto object-cover mb-4 border-4 border-slate-100" onError={(e) => { e.currentTarget.src = avatarFallback; }} />
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
            ...
            <div className="bg-slate-50 p-4 rounded-lg flex justify-center">
              {qrCodeUrl ? (
                ...
                <div className="w-32 h-32 bg-slate-200 rounded-lg flex items-center justify-center"><p className="text-xs text-slate-400 text-center">QR code<br/>non disponible</p></div>
              )}
            </div>
            <p className="text-xs text-slate-500 text-center mt-3 font-mono">{student.matricule}</p>
          </div>
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
            <h3 className="text-base font-semibold text-slate-900 mb-5 flex items-center gap-2"><GraduationCap size={18} className="text-[#FF6B00]" /> Informations académiques</h3>
        ...
        <div className="lg:col-span-2 space-y-5">
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
            <h3 className="text-base font-semibold text-slate-900 mb-5 flex items-center gap-2"><User size={18} className="text-[#FF6B00]" /> Informations personnelles</h3>
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
            <h3 className="text-base font-semibold text-slate-900 mb-5 flex items-center gap-2"><Mail size={18} className="text-[#FF6B00]" /> Contact</h3>
            <div className="grid grid-cols-2 gap-5">
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
            <h3 className="text-base font-semibold text-slate-900 mb-5 flex items-center gap-2"><User size={18} className="text-[#FF6B00]" /> Parent / Tuteur</h3>
            <div className="grid grid-cols-2 gap-5">
              <div><p className="text-xs text-slate-500 mb-1">Nom complet</p><p className="text-sm font-medium text-slate-900">{student.parent_name || 'Non renseigné'}</p></div>
              <div><p className="text-xs text-slate-500 mb-1">Téléphone</p><p className="text-sm font-medium text-slate-900">{student.parent_phone || 'Non renseigné'}</p></div>
              <div className="col-span-2"><p className="text-xs text-slate-500 mb-1">Email</p><p className="text-sm font-medium text-slate-900">{student.parent_email || 'Non renseigné'}</p></div>
            </div>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
