...
export default function CenseurGradesEntryPage() {
  ...
  const handleScoreChange = (studentId: number, value: string) => {
    setGrades(prev => prev.map(g => {
      if (g.student_id === studentId) {
        ...
        return {
          ...g,
          score: score !== null && !isNaN(score) ? Math.min(20, Math.max(0, score)) : null,
          status: score !== null ? 'draft' : 'not_graded'
        };
      }
      return g;
    }));
  ...
  const handleSave = async (status: 'draft' | 'validated') => {
    ...
    if (status === 'validated') {
      const ok = await confirm({
        ...
        message: `Vous êtes sur le point de valider ${grades.filter(g => g.score !== null).length} notes. Elles seront définitives et visibles par les étudiants.`,
        confirmText: 'Valider',
        cancelText: 'Annuler',
        variant: 'success',
        icon: 'check'
    ...
    try {
      ...
      toast.success(
        status === 'validated' ? 'Notes validées avec succès' : 'Brouillon sauvegardé'
      );
      setHasChanges(false);
      loadGrades();
    } catch (error: any) {
  ...
  const filteredGrades = grades.filter(g => {
    ...
    return name.includes(search.toLowerCase()) || g.matricule.toLowerCase().includes(search.toLowerCase());
  });
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1">Saisissez les notes par cours et par classe</p>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div>
            <label className="block text-xs font-medium text-slate-700 mb-1">
              <Calendar size={12} className="inline mr-1" /> Session *
            </label>
            <select
          ...
          <div>
            <label className="block text-xs font-medium text-slate-700 mb-1">
              <Users size={12} className="inline mr-1" /> Classe *
            </label>
            <select
          ...
          <div>
            <label className="block text-xs font-medium text-slate-700 mb-1">
              <BookOpen size={12} className="inline mr-1" /> Cours *
            </label>
            <select
            ...
            >
              <option value="">{!selectedClassId ? 'Choisissez classe...' : 'Sélectionner...'}</option>
                ...
                <table className="w-full">
                  <thead className="bg-gradient-to-r from-slate-50 to-slate-100 border-b border-slate-200">
                    <tr>
                      ...
                      <th className="text-center py-3 px-4 text-xs font-semibold text-slate-600 uppercase w-40">Note /20</th>
                      ...
                      <tr key={grade.student_id} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="py-3 px-4">
                          <input
                            ...
                            className={`w-full px-3 py-1.5 border rounded-lg text-sm text-center font-bold focus:outline-none focus:ring-2 focus:ring-[#FF6B00] ${
                              grade.score !== null ? getScoreColor(grade.score) : 'border-slate-200'
                            }`}
                          />
                        </td>
