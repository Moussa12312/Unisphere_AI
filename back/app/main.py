    ...
    try:
        ...
        if not tronc:
            ...
            for university in universities:
                ...
                db.add(new_tronc)
            db.commit()
            print(f"[OK] Filière 'Tronc commun' créée pour {len(universities)} université(s)")
        else:
            print("[OK] Filière 'Tronc commun' existe déjà")
    except Exception as e:
        print(f"[ERROR] Erreur création tronc commun: {e}")
        db.rollback()
    finally:
        db.close()
