...
ALL_ROLES = ("admin", "secretary", "teacher", "student", "censeur", "accountant", "guard", "alumni")
...
def get_unread_count(
    db: Session = Depends(get_db),
    current_user: User = Depends(require_role("admin", "secretary", "censeur", "accountant", "guard", "teacher", "student", "alumni"))
):
...
def broadcast_message(
    ...
    subject = data.get("subject")
    content = data.get("content")
    level = data.get("level")          # ✅ AJOUTÉ : ex "L1", "L2", "M1"...
    filiere = data.get("filiere")      # ✅ AJOUTÉ : filtrage optionnel par filière
    ...
    if target_type == "all_students":
        query = db.query(User).join(Student, Student.user_id == User.id).filter(
            User.university_id == current_user.university_id,
            User.role == "student"
        )
        # ✅ AJOUTÉ : filtrage par niveau (L1, L2...) et/ou filière
        if level:
        ...
        recipients = db.query(User).filter(
            User.university_id == current_user.university_id,
            User.role.in_(("admin", "secretary", "accountant", "censeur", "guard", "alumni"))
        ).all()
    ...
    elif target_type == "all_censeurs":
        recipients = db.query(User).filter(
            User.university_id == current_user.university_id,
            User.role == "censeur"
        ).all()
    elif target_type == "all_accountants":
        recipients = db.query(User).filter(
            User.university_id == current_user.university_id,
            User.role == "accountant"
        ).all()
    elif target_type == "all_guards":
        recipients = db.query(User).filter(
            User.university_id == current_user.university_id,
            User.role == "guard"
        ).all()
    elif target_type == "all_alumni":
        recipients = db.query(User).filter(
            User.university_id == current_user.university_id,
            User.role == "alumni"
        ).all()
