...
import { User, Mail, Phone, Shield, Save, Briefcase, ScanLine, AlertTriangle } from 'lucide-react';
  ...
  const fetchStats = async () => {
    try {
      const [attendanceRes, incidentsRes] = await Promise.all([
        api.get('/api/v1/attendance/today/stats').catch(() => ({ data: { present: 0, late: 0 } })),
        api.get('/api/v1/incidents/').catch(() => ({ data: [] }))
      ]);
      const incidents = Array.isArray(incidentsRes.data) ? incidentsRes.data : (incidentsRes.data?.data || []);
      setStats({
        scans_today: (attendanceRes.data?.present || 0) + (attendanceRes.data?.late || 0),
        ...
        <div className="flex items-center gap-5">
          ...
          <div className="hidden md:flex gap-3">
            <div className="bg-white/20 backdrop-blur-sm rounded-xl p-3 text-center min-w-[100px]">
              <ScanLine size={18} className="mx-auto mb-1" />
              <p className="text-2xl font-bold">{stats.scans_today}</p>
              <p className="text-xs opacity-80">Scans aujourd'hui</p>
            </div>
            <div className="bg-white/20 backdrop-blur-sm rounded-xl p-3 text-center min-w-[100px]">
              <AlertTriangle size={18} className="mx-auto mb-1" />
              <p className="text-2xl font-bold">{stats.incidents}</p>
              <p className="text-xs opacity-80">Incidents</p>
            </div>
          </div>
        </div>
      </div>
