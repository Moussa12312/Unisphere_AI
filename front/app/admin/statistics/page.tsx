...
export default function StatisticsPage() {
  ...
  const loadAllStats = async () => {
    ...
    try {
      ...
      const healthCheck = await api.get('/api/v1/health').catch(() => null) || await api.get('/health').catch(() => null);
      ...
      if (overviewRes) {
        ...
        setBackendError(`Backend inaccessible. Vérifiez que le serveur tourne sur ${API_BASE_URL || 'port 8000'}`);
        setLoading(false);
        return;
      } else {
      ...
      if (error.code === 'ERR_NETWORK') {
        setBackendError('Backend inaccessible. Vérifiez que le serveur tourne sur ${API_BASE_URL}');
